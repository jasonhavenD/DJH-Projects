...
u.Text = u.BaseComponent.extend({
    ...
    init: function () {
        ...
        if (this._input) {
            ...
            if(u.isIE8){
                this._input.addEventListener('propertychange', this.boundUpdateClassesHandler);
            }
        ...
        if (meta[key]['default']) {
            ...
            if (typeof defaults === 'object'){
                for (var k in defaults) {
                    if (k == 'value'){
                        if (typeof defaults[k] === 'function')
                            this.data[key].value = this.formatValue(key,defaults[k]());
                        else
                            this.data[key].value = this.formatValue(key,defaults[k]);
                    }
                    else {
                ...
                if (typeof defaults === 'function')
                    this.data[key].value = this.formatValue(key, defaults());
                else
                    this.data[key].value = this.formatValue(key,defaults);
            }
        }
...
u.TimeAdapter = u.BaseAdapter.extend({
    initialize: function (options) {
        ...
        if (this.options.type == 'u-clockpicker')
