...
<title>
	<c:if test="${userinfor.userId != null && '' != userinfor.userId }">修改</c:if>
...
<body>
	...
	<table class="tableStyle" formMode="transparent">
		...
		<tr>
			...
			<td>
			<s:if test="%{userinfor.userId != null && '' != userinfor.userId}">
				<div disabled="true" class="selectTree validate[required]" selectedValue="<s:property value="userinfor.organization.orgId"/>" name="userinfor.organization.orgId" id="department" url="<%=path%>/getDepartmentsTree.action"></div><span class="star">*</span>
			</s:if>
			<s:else>
				<div disabled="true" class="selectTree validate[required]" <% String orgid = request.getParameter("orgid"); if(orgid != null && !"".equals(orgid) && !"0".equals(orgid)){%>selectedValue="<%=orgid%>"<%}%> name="userinfor.organization.orgId" id="department" url="<%=path%>/getDepartmentsTree.action"></div><span class="star">*</span>
			</s:else>
			</td>
		</tr>
		<tr>
...
function closeWin(){
	...
	//刷新数据
	top.frmright.refreshChild(update,<%=request.getParameter("gridId")%>);
	//关闭窗口
	top.Dialog.close();
}
</script>
