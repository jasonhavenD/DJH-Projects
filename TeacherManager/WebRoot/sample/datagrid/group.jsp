...
<body>
<div class="box1">
    分组时只能对已经加载的数据分组。第二个例子，在分组时对每组数量做了统计，并设置了更个性化的分组效果。
</div>
		...
		function initComplete(){
			 ...
			 grid1 = $("#grid1").quiGrid({
		        columns: [ 
		            ...
		            { display: '性别', name: 'sex',      align: 'center', width: "25%"},
		            { display: '部门', name: 'deptName', align: 'center', width: "25%", isSort:false},
		            { display: '学历', name: 'degree',   align: 'center', width: "25%" }
		        ], 
		        groupColumnName: "sex", groupColumnDisplay: "性别",
		        data: gridData, pageSize: 10, sortName: 'id',rownumbers:true,
		    ...
		    grid2 = $("#grid2").quiGrid({
		        ...
		        groupColumnName: "sex", groupColumnDisplay: '性别',
		        //可以使用groupColumnDisplay，也可以在return语句中自己拼语句
		        groupRender: function(groupValue, groupData, groupColumnDisplay){
		            if("女" == groupValue){
		                return "<font color=red>美女" + "【共" + groupData.length + "人】</font>";
		            }else{
		                return "帅哥【共" + groupData.length + "人】";
		            }
		        },
		        data: gridData, pageSize: 10, sortName: 'id',rownumbers:true,
		        height: '450', width:"100%"
