...
function stublur() {//当第一个下拉框的选项发生改变时调用该函数
	var ptn = document.getElementById("stuNumber").value; //获取第一级菜单的值
	
	sendRequest("../stuNumber_uniqueness?stuNumber=" + ptn); //发送参数到SERVLET
	...
	if (XMLHttpReq.readyState == 4) {
		if (XMLHttpReq.status == 200) {
			...
			if (xValue == "true") {//如果不为空就置空
				...
				obj.innerHTML="该学号已存在";
				$('#stuNumber').css('border','1px solid red');
				//document.getElementById("patentNumberError").setAttribute("style","font-size:12px;color:red;");
				documnet.getElementById("stuNumberError").style.color="red";
			}else{
				if(document.getElementById("stuNumber").value.length==0)
				{
					obj.innerHTML="学号不可为空";
					obj.style.color="red";
				}
				else
				{
				obj.innerHTML="学号可用";
				$('#stuNumber').css('border','1px solid green');
