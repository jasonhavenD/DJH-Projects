...
//onblur事件
function prescieblur() {//当第一个下拉框的选项发生改变时调用该函数
	var ptn = document.getElementById("projectNo").value; //获取第一级菜单的值
	
	sendRequest("../PreScient_uniqueness?projectNo=" + ptn); //发送参数到SERVLET
...
function progessRequest() {
	if (XMLHttpReq.readyState == 4) {
		if (XMLHttpReq.status == 200) {
			...
			var obj = document.getElementById("projectNoError");
