...
function stublur() {//当第一个下拉框的选项发生改变时调用该函数
	...
	sendRequest("../inuniqueness?comNumber=" + ptn); //发送参数到SERVLET
...
function progessRequest() {
	if (XMLHttpReq.readyState == 4) {
		if (XMLHttpReq.status == 200) {
			...
			var s = xmlDoc.getElementsByTagName("competition");//获取XML节点
			...
			if (xValue == "true") {//如果不为空就置空
				...
				document.getElementById("comNumberError").setAttribute(
