...
function stublur() {//当第一个下拉框的选项发生改变时调用该函数
	var ptn = document.getElementById("rprojectNo").value; //获取第一级菜单的值
	
	sendRequest("../PreRevolut_uniqueness?rprojectNo=" + ptn); //发送参数到SERVLET
...
function progessRequest() {
	if (XMLHttpReq.readyState == 4) {
		if (XMLHttpReq.status == 200) {
			...
			var s = xmlDoc.getElementsByTagName("pre");//获取XML节点
			var obj = document.getElementById("rprojectNoError");
			var xValue = s[0].childNodes[0].firstChild.nodeValue;
			if (xValue == "true") {//如果不为空就置空
				obj.innerHTML="项目编号已存在";
				/*$('#rprojectNo').css('border','1px solid red');*/
				
				obj.style.color="red";
				
			}else{
				if(document.getElementById("rprojectNo").value.length == 0){
					obj.innerHTML="请输入项目编号";
					/*$('#rprojectNo').css('border','1px solid red');*/
					obj.style.color="red";
				} else{
					obj.innerHTML="项目编号可用";
					/*$('#rprojectNo').css('border','1px solid green');*/
