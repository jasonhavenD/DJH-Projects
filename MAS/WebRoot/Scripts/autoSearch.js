...
function findNames(userId) {
	...
	if (inputField.value.length > 0) {
		...
		var url = "autoSearch.action?role=teacher&userId="+userId+"&idname="+value;
		//此处escape函数可以去掉，escape是采用ISO Latin字符集对指定的字符串进行编码。 
		o.open("GET", url, true);
		o.onreadystatechange = function() {
			if (o.readyState == 4) { // 判断对象状态  
				if (o.status == 200) { // 信息已经成功返回，开始处理信息 
					setNames(o.responseXML.getElementsByTagName("ids"),o.responseXML.getElementsByTagName("names"));
				} else { //页面不正常  
...
function setNames(ids,names) {
	...
	if(size >= 0){
		...
		if(width < 250){
			cell.setAttribute("width", "125px");
			cell2.setAttribute("width", "125px");
		}
		else{
	...
	for ( var i = 0; i < size; i++) {
		...
		nextNode = nextNode.replace("$1","<span style='display:inline;color:red;'>").replace("$2","</span>");
		nextNode2 = nextNode2.replace("$1","<span style='display:inline;color:red;'>").replace("$2","</span>");
...
function gettable(inputField) {
	...
	var width = inputField.offsetWidth;
	if(width < 250)
		completeDiv.style.left = (pos.left-(250-width)/2) + "px";
	else
		completeDiv.style.left = pos.left + "px";
	if(size > 10)
		completeDiv.style.height = "231px";
	else
		completeDiv.style.height = ((size+1)*21)+1+"px";
	if(size == 0)
		completeDiv.style.height = "43px";
	if(size >= 0){
