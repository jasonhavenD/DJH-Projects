...
//添加赛项
function insertStuCption() {
		if (document.getElementById("rank").value.length == 0) {
		confirm("排名选项不能为空");
	} else if (confirm("是否添加")) {
			document.getElementById("insertStuCption").submit();
		}
}
//确认修改赛项
function modifiStuCption() {

    if(document.getElementById("names").value.length == 0){
         document.getElementById("names").focus();
		 confirm("成员工号选项不能为空");
     }
     else if (document.getElementById("rank").value.length == 0) {
     document.getElementById("rank").focus();
		confirm("排名选项不能为空");
	} else if (confirm("是否保存")) {
			document.getElementById("modifiStuCption").submit();
		}
}
</script>
	...
	<body style="background-color:#e4dfd9" >
		...
		<div class="alert alert-info">
			...
			<b class="tip"></b>修改竞赛获奖成员信息
		</div>
		<form action="modifiStuCption.action" id="modifiStuCption"
			method="post">
			...
			<table class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<tr>
						...
						<td>
							<input name="studentcoepetition.competition.comName"
						...
						<td>
							<div id="abc" style="display: inline; float: left;">
								<input type="text" name="studentcoepetition.student.stuNumber"
									value='<s:property value="studentcoepetition.student.stuNumber" />'
									style="background: transparent;" placeholder="输入学号或姓名搜索"
					...
					<tr>
						...
						<td colspan="5">
							<input name="studentcoepetition.rank" id="rank" type="text"
								value='<s:property value="studentcoepetition.rank"/>'
				...
				<tfoot>
					<tr>
						<td colspan="99">
							...
							<a href="javascript:modifiStuCption()"
...
function findNames(userId) {
	...
	if (inputField.value.length > 0) {
		...
		var url = "autoSearch.action?role=student&userId=" + userId
				+ "&idname=" + value;
