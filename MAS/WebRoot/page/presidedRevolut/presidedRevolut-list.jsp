...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript"
			src="${pageContext.request.contextPath}/Scripts/ajax/MajorByDno.js"></script>
<script type="text/javascript"
		...
		<script type="text/javascript">
			...
			ptname = encodeURI(ptname);
		 	...
		 	pprojecJibie = encodeURI(pprojecJibie);
				window.location.href = "findPrer.action?"
				+ "pmodel.id=" + '${pmodel.id}'
				+ "&pmodel.name="+pname
			...
			ptname = encodeURI(ptname);
		 	...
		 	pprojecJibie = encodeURI(pprojecJibie);
		 	pprojecJibie = encodeURI(pprojecJibie);
				window.location.href = "exportExcel.action?exportName=PresidedRevolution"
				+ "&prermodel.id=" + '${pmodel.id}'
				+ "&prermodel.name="+pname
				+ "&prermodel.tno="+'${pmodel.tno}'
				+ "&prermodel.tname="+ptname
				+ "&prermodel.departmentId=" + '${pmodel.departmentId}'
				+ "&prermodel.majorId=" + '${pmodel.majorId}'
				+ "&prermodel.year=" + '${pmodel.year}'
				+ "&prermodel.projecType=" + pprojecType
				+ "&prermodel.projecJibie=" + pprojecJibie;

			}
//导入
//上传附件弹出框
function upFile() {
	document.getElementById("iframe").src = "attachFileList.action?importName=prer";
	$('#alert-win').dialog( {
		width : 450,
		height : 240,
		buttons : {
...
function importExcel() {
	window.location.href = "importExcel.action?importName=PresidedRevolution&excelFile="+getPath(document.getElementById("excelFile"));
}
		</script>
	...
	<body style="background-color:#e4dfd9" >
		<div class="alert alert-info">
			...
			<b class="tip"></b>查询教改项目信息
		</div>
		<form action="findPrer.action?page=1&rows=10" id="form1" name="form1"
			...
			<table class="table table-striped table-bordered table-condensed">
				...
				<tbody id="mytable">
					...
					<tr>
						...
						<td>
							<select size="1" id="tno" name="pmodel.Tno" class="teacher">
								<option value="">
									选择教师
								</option>
						...
						<td>
							<select size="1" id="teacher" name="pmodel.Tname" class="teacher">
								<option value="">
									选择教师
								</option>
								<s:iterator value="teacherList" var="teavar">
						...
						<td>
							<select size="1" id="coll" name="pmodel.departmentId"
					...
					<tr>
						...
						<td>
							<select name="pmodel.ProjecType" id="projecType">
								...
								<s:iterator value="rprojecTypeList" var="projecTypevar">
									<option value="<s:property/>"
						...
						<td>
							<select name="pmodel.ProjecJibie" id="projecJibie">
								...
								<s:iterator value="rprojecJibieList" var="proJivar">
									<option value="<s:property/>"
						...
						<td>
							<select size="1" id="major" name="pmodel.majorId" class="major">
								<option value="">
									选择专业
								</option>
				...
				<tfoot>
					<tr>
						<td colspan="5">
							...
							<a href="#" onclick="exportExcel()" class="btn btn-primary add">导出主持教改项目列表</a>
							&nbsp;&nbsp;
							<span style="color: red;">(先查询，再导出)</span>

						</td>
						<td align="center">
							<a id="add" href="addPrer.action?Pno=0" style=""
		...
		<table style="white-space: normal;" class="table table-striped table-bordered table-condensed">
			...
			<tbody>
				...
				<s:iterator value="prerList" id="prer" status="L">
					<tr>
						<td>
							<s:property value="#L.index+1" />
						</td>
						<td>
							<s:property value="rprojectNo" />
						</td>
						<td>
							<s:property value="rprojectName" />
						</td>
						<td>
							<s:property value="teacher.tname" />
						</td>
						<td>
							<s:property value="rprojecType" />
						</td>
						<td>
							<s:property value="rprojecJibie" />
						</td>
						<td>
						...
						<td>
							<s:property value="rpartTeacherNum" />
						</td>
						<td>
							<s:property value="rcost" />
						</td>
						<td>
						...
						<td>
							<span class="edit"> <a class="btn btn-mini btn-primary"
								href="editToPrer.action?Pno=<s:property value="rprojectNo"/>">
									修改 </a> </span>
							<span class="del"> <a class="btn btn-mini btn-primary"
								href="deletePrer.action?Pno=<s:property value="rprojectNo"/>"
